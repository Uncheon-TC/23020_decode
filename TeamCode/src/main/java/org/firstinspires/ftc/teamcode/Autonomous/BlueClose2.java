...
@Autonomous(name = "AutoBlueClose2 30s/60s", group = "32020 AUTO")
public class BlueClose2 extends OpMode {
    ...
    // 현재 로봇 Pose를 이용해 블루 골대를 계속 조준한다.
    ...
    public void init() {
        ...
        ninthAndTenthPath = follower.pathBuilder()
                ...
                .setLinearHeadingInterpolation(Math.toRadians(153), Math.toRadians(200))
    ...
    private void updateAutomaticAim() {
        ...
        // 현재 위치에서 블루 골대까지의 거리와 로봇 이동 속도를 반영한다.
    ...
    private void updateTelemetry() {
        ...
        if (shotResult != null) {
            // 현재 로봇 위치에서 블루 골대까지 계산된 거리를 표시한다.
