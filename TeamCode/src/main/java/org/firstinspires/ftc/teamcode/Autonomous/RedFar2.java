...
public class RedFar2 extends OpMode {
    ...
    private PathChain firstPath, secondPath, thirdPath, fourthPath, fifthPath, sixthPath, seventhPath, eighthPath, ninthPath, tenthPath, eleventhPath, twelfthPath;
    ...
    public void init() {
        ...
        firstPath = follower.pathBuilder()
                .addPath(new BezierCurve(STARTING_POSE, PATH1_CONTROL, PATH1_POSE))
                .setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0))
                .build();

        secondPath = follower.pathBuilder()
                .addPath(new BezierLine(PATH1_POSE, PATH2_POSE))
                .setConstantHeadingInterpolation(Math.toRadians(0))
                .build();

        thirdPath = follower.pathBuilder()
                .addPath(new BezierLine(PATH2_POSE, PATH3_POSE))
                .setConstantHeadingInterpolation(Math.toRadians(0))
                .build();
    ...
    public void loop() {
        ...
        switch (autoState) {
            ...
            // ---- horizontal ball: collect -> shoot ----
            case DRIVE_TO_PATH1: // collects artifact
                if (!follower.isBusy()) {
                    follower.followPath(secondPath);
                    shooter.spinUp();
                    autoState = AutoState.DRIVE_TO_PATH2;
                }
                break;

            case DRIVE_TO_PATH2: // -> shooting
                if (!follower.isBusy()) {
                    beginShotSequence();
                    autoState = AutoState.SHOOT_AT_PATH2;
                }
                break;
