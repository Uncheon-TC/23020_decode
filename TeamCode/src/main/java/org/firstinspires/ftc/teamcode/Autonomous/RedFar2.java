...
@Autonomous(name = "RedClose 30s", group = "23020 AUTO")
public class RedClose2 extends OpMode {
    ...
    private static final double RETURN_SHOT_DELAY_SECONDS = 0.1;
    ...
    private static final Pose THIRD_POSE =
            new Pose(122, 60, Math.toRadians(0)); // 두번째 줄 유물 먹는 위치
    ...
    private static final Pose FIFTH_POSE =
            new Pose(127, 63, Math.toRadians(0)); // 두번째 이후 게이트 오픈 위치
    private static final Pose FIFTH_CURVE =
            new Pose(117,62, Math.toRadians(0)); // 두번째 이후 게이트 오픈하러가는 커브 체크포인트
    private static final Pose SIXTH_POSE = // 두번째 이후 게이트에서 인테이크 하는 위치
            new Pose(131, 62, Math.toRadians(32));
    ...
    public void init() {
        ...
        sixthPath = follower.pathBuilder()
                .addPath(new BezierCurve(FORTH_POSE, THIRD_CURVE, FIRST_POSE))
                .setLinearHeadingInterpolation(
                FORTH_POSE.getHeading(), FIRST_POSE.getHeading())
                .build();
        seventhPath = follower.pathBuilder()
                .addPath(new BezierCurve(FIRST_POSE, FIFTH_CURVE,FIFTH_POSE))
                .setLinearHeadingInterpolation(
        ...
        eighthPath = follower.pathBuilder()
                .addPath(new BezierCurve(SIXTH_POSE, FIFTH_CURVE, FIRST_POSE))
                .setLinearHeadingInterpolation(
